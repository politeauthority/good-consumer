...
class ModelCompanyTypes( object ):
  ...
  def getIDsByName( self, type_names, create_if_not_exists = False ):
    ...
    for name in type_names:
      qry = """SELECT * FROM `%s`.`companies_types` WHERE `name` = "%s";  """ % ( self.db_name, name['name'] )
      result = Mysql.ex( qry )
      if len( result ) == 0:
        type_id = self.create( name['name'], name['wikipedia'] )
      else:
        type_id = result[0]['companies_type_id']
  ...
  def create( self, name, wikipedia_url, desc = None ):
    ...
    exists = Mysql.ex( qry )
    if len( exists ) > 0:
      Debugger.write( 'exists', exists[0]['companies_type_id'] )
      return exists[0]['companies_type_id']
    args = {
      'name'      : name,
      'wikipedia' : wikipedia_url,
      'desc'      : desc,
    }
    Mysql.insert( 'companies_types', args );
    qry = """SELECT * FROM `%s`.`companies_types` WHERE `wikipedia`="%s";""" % ( self.db_name, wikipedia_url )
    companies_type_id = str( Mysql.ex( qry )[0]['company_type_id'] )
    Debugger.write( 'companies_type_id', company_type_id )
    return companies_type_id
